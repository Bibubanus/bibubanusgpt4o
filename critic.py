...
class Critic:
    def evaluate_graph(self, reasoning_graph) -> float:
        ...
        if len(degrees) > 2:
            max_deg = max(degrees)
            # Correct centralization formula for directed graphs
            # Freeman's centralization index: sum of differences divided by theoretical maximum
            sum_diff = sum(max_deg - d for d in degrees)
            # Maximum possible centralization for n nodes is (n-1)*(n-2) for directed graphs
            max_possible = (len(degrees) - 1) * (len(degrees) - 2)
            centralisation = sum_diff / max_possible if max_possible > 0 else 0.0
        else:
            # For graphs with 2 or fewer nodes, centralization is undefined or 0
            centralisation = 0.0
